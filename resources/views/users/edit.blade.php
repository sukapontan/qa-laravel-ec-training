...
<div class="container">
    ...
    <form class="edit_user" enctype="multipart/form-data" action="{{ route('users.update', $user->id) }}" accept-charset="UTF-8" method="post">
        ...
        <div class="d-flex justify-content-around py-5 col-sm-8 col-auto container">
            <input type="submit" class="btn btn-primary px-1 col-md-2 col-2" value="修正" onclick="return FixCheck()"></input>
    ...
    <form class="delete_user" enctype="multipart/form-data" action="{{ route('users.destroy', $user->id) }}" accept-charset="UTF-8" method="post">
        @csrf
        @method('DELETE')
        <input type="submit" class="btn btn-danger px-5 -col-md-2 col-15" value="退会" onclick="return WithdrawalCheck()"></input>
    </form>
</div>
...
<!-- 修正ボタン押下後、修正内容を確認する処理 -->
<script type="text/javascript">
    function FixCheck() {
        var checked = confirm("修正内容にお間違いはないですか？");
        if (checked == true) {
            return true;
        } else {
            return false;
